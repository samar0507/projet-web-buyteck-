...
function totalreclamation()
{
    $db = config::getConnexion();
    $sth = $db->prepare('SELECT count(*) as total from reclamation');
    $sth->execute();
    $total=$sth->fetchColumn();
    return$total;}
    
    function totalreclamationetat($etat)
    {
        $db = config::getConnexion();
        $sql="SELECT count(*)  from reclamation where $etat='treated'";
        $sth = $db->prepare("$sql");
        $sth->execute();
        $total=$sth->fetchColumn();
        return$total;
